...
function makeQ(difficulty: number): Q {
  ...
  if (tier === 0) {
    ...
    const s = v * t
    const answer = `${s} km`
    const cands = [s + v, s - v, s + t * 10, s + v * 2, s - t * 10, s * 2].filter(n => n > 0 && n !== s).map(n => `${n} km`)
    return { question: `Hastighet ${v} km/h, tid ${t} h — Sträcka?`, answer, options: uniqueFourStr(answer, () => cands[Math.floor(Math.random() * cands.length)]) }
  } else if (tier === 1) {
    // Find speed: v = s ÷ t
    const t = TIMES[Math.floor(Math.random() * TIMES.length)]
    const v = SPEEDS[Math.floor(Math.random() * SPEEDS.length)]
    const s = v * t
    const answer = `${v} km/h`
    const cands = [v + 10, v - 10, v + 20, v - 20, v + 5, s].filter(n => n > 0 && n !== v).map(n => `${n} km/h`)
    return { question: `Sträcka ${s} km, tid ${t} h — Hastighet?`, answer, options: uniqueFourStr(answer, () => cands[Math.floor(Math.random() * cands.length)]) }
  } else {
    // Find time: t = s ÷ v
    const v = SPEEDS[Math.floor(Math.random() * SPEEDS.length)]
    const t = TIMES[Math.floor(Math.random() * TIMES.length)]
    const s = v * t
    const answer = t === 1 ? '1 timme' : `${t} timmar`
    const wrongs = TIMES.filter(n => n !== t).map(n => n === 1 ? '1 timme' : `${n} timmar`)
    return { question: `Sträcka ${s} km, hastighet ${v} km/h — Tid?`, answer, options: uniqueFourStr(answer, () => wrongs[Math.floor(Math.random() * wrongs.length)]) }
  }
}

export const SpeedDistTimeGame = memo(function SpeedDistTimeGame({ onExit, onWin }: Props) {
  ...
  const [bestScore] = useState(() => Number(localStorage.getItem('k0509_sdt_best') ?? 0))
  ...
  const nextRound = useCallback((r: number) => {
    if (r >= ROUNDS) {
      ...
      const prev = Number(localStorage.getItem('k0509_sdt_best') ?? 0)
      if (s > prev) localStorage.setItem('k0509_sdt_best', String(s))
      onWin(s * 18, s * 54)
      setPhase('done')
      audio.achievement()
      return
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div className={styles.gameHeader}>
        ...
        <span className={styles.gameTitle}>🚗 Hastighet</span>
      ...
      {phase === 'ready' && (
        <div style={{ padding: '24px 16px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ fontFamily: 'var(--ff-head)', fontSize: 18, fontWeight: 900, color: '#fff' }}>Hastighet, sträcka, tid</div>
          <div style={{ fontSize: 12, color: 'var(--t3)', maxWidth: 260, lineHeight: 1.6 }}>
            s = v × t. Räkna ut sträcka, hastighet och tid! {TIME_LIMIT} sek, {ROUNDS} ronder.
          </div>
      ...
      {phase === 'play' && (
        <div style={{ padding: '0 14px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {q.options.map((opt, i) => (
              <button key={i} onClick={() => answer(opt)} style={{ height: 64, borderRadius: 16, fontSize: 15, fontWeight: 900, background: 'rgba(56,189,248,.1)', color: accent, border: '2px solid rgba(56,189,248,.3)', cursor: 'pointer', padding: '0 6px' }}>{opt}</button>
            ))}
          </div>
        </div>
      )}
