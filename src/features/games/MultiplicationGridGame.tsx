...
function makeQ(difficulty: number): Q {
  ...
  if (tier === 0) {
    // Simple: a × b = ? (tables 2-5)
    const a = 2 + Math.floor(Math.random() * 4)
    const b = 2 + Math.floor(Math.random() * 9)
    const answer = a * b
    const cands = [a * (b + 1), a * (b - 1), (a + 1) * b, (a - 1) * b, answer + a, answer - a].filter(v => v > 0 && v !== answer)
    return { question: `${a} × ${b} = ?`, answer, options: uniqueFourNum(answer, () => cands[Math.floor(Math.random() * cands.length)]) }
  } else if (tier === 1) {
    // Medium: a × b = ? (tables 6-9)
    const a = 6 + Math.floor(Math.random() * 4)
    const b = 6 + Math.floor(Math.random() * 5)
    const answer = a * b
    const cands = [a * (b + 1), a * (b - 1), (a + 1) * b, answer + a, answer - b, answer + 10].filter(v => v > 0 && v !== answer)
    return { question: `${a} × ${b} = ?`, answer, options: uniqueFourNum(answer, () => cands[Math.floor(Math.random() * cands.length)]) }
  } else {
    // Hard: missing factor — ? × b = c
    const a = 3 + Math.floor(Math.random() * 8)
    const b = 3 + Math.floor(Math.random() * 8)
    const c = a * b
    const answer = a
    const cands = [a + 1, a - 1, a + 2, b, c - b, Math.floor(c / (b + 1))].filter(v => v > 0 && v !== a)
    return { question: `? × ${b} = ${c}`, answer, options: uniqueFourNum(answer, () => cands[Math.floor(Math.random() * cands.length)]) }
  }
}

export const MultiplicationGridGame = memo(function MultiplicationGridGame({ onExit, onWin }: Props) {
  ...
  const [bestScore] = useState(() => Number(localStorage.getItem('k0509_multigrid_best') ?? 0))
  ...
  const nextRound = useCallback((r: number) => {
    if (r >= ROUNDS) {
      ...
      const prev = Number(localStorage.getItem('k0509_multigrid_best') ?? 0)
      if (s > prev) localStorage.setItem('k0509_multigrid_best', String(s))
      onWin(s * 16, s * 48)
      setPhase('done')
      audio.achievement()
      return
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div className={styles.gameHeader}>
        ...
        <span className={styles.gameTitle}>✖️ Tabellgrid</span>
      ...
      {phase === 'ready' && (
        <div style={{ padding: '24px 16px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ fontFamily: 'var(--ff-head)', fontSize: 18, fontWeight: 900, color: '#fff' }}>Tabellgrid</div>
          <div style={{ fontSize: 12, color: 'var(--t3)', maxWidth: 260, lineHeight: 1.6 }}>
            Multiplikationstabeller 2–9 och saknade faktorer! {TIME_LIMIT} sek, {ROUNDS} ronder.
          </div>
      ...
      {phase === 'play' && (
        <div style={{ padding: '0 14px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {q.options.map((opt, i) => (
              <button key={i} onClick={() => answer(opt)} style={{ height: 64, borderRadius: 16, fontSize: 24, fontWeight: 900, background: 'rgba(251,146,60,.1)', color: accent, border: '2px solid rgba(251,146,60,.3)', cursor: 'pointer' }}>{opt}</button>
            ))}
          </div>
        </div>
      )}
      ...
      {phase === 'feedback' && (
        <div style={{ padding: '32px 16px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ fontSize: 24, color: accent, fontWeight: 900 }}>{q.question} {q.answer}</div>
