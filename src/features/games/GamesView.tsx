...
const GAMES = [
  ...
  { id: 'numbase' as const, emoji: '🔢', name: 'Talbaser', desc: 'Binärt, decimalt och hex — 8 sek, 12 ronder!', reward: '🪙0-192', hot: true },
  { id: 'coordgame' as const, emoji: '📍', name: 'Koordinater', desc: 'Koordinatsystemet och kvadranter — 8 sek, 12 ronder!', reward: '🪙0-180', hot: false },
  { id: 'negnum' as const, emoji: '➖', name: 'Negativa tal', desc: 'Räkna med negativa tal — 7 sek, 12 ronder!', reward: '🪙0-180', hot: true },
  { id: 'unitconv' as const, emoji: '⚖️', name: 'Enhetsomvandling', desc: 'Längd, massa, tid, volym — 8 sek, 12 ronder!', reward: '🪙0-180', hot: false },
  { id: 'ratio' as const, emoji: '⚡', name: 'Proportioner', desc: 'Förhållanden, saknade termer — 8 sek, 12 ronder!', reward: '🪙0-192', hot: true },
  { id: 'speedtime' as const, emoji: '🚗', name: 'Hastighet', desc: 's = v × t, räkna sträcka/tid/hastighet — 10 sek!', reward: '🪙0-180', hot: false },
  { id: 'exponents' as const, emoji: '🔋', name: 'Potenser', desc: 'Kvadrater, kuber och rötter — 7 sek, 12 ronder!', reward: '🪙0-192', hot: true },
  { id: 'swwords' as const, emoji: '🇸🇪', name: 'Svenska', desc: 'En/ett, plural och verbböjning — 7 sek, 12 ronder!', reward: '🪙0-180', hot: false },
  { id: 'algword' as const, emoji: '🔣', name: 'Algebraekvationer', desc: 'Lös x+a=b, ax=b och ax+b=c — 10 sek, 12 ronder!', reward: '🪙0-204', hot: true },
  { id: 'swgram' as const, emoji: '📚', name: 'Grammatik', desc: 'Komparativ, bestämd form & prepositioner — 8 sek!', reward: '🪙0-180', hot: false },
  { id: 'multigrid' as const, emoji: '✖️', name: 'Tabellgrid', desc: 'Multiplikationstabeller 2–9 och saknade faktorer — 8 sek!', reward: '🪙0-192', hot: true },
  { id: 'scifact' as const, emoji: '🔬', name: 'NO-fakta', desc: 'Fysik, kemi och biologi — 9 sek, 10 ronder!', reward: '🪙0-170', hot: false },
  { id: 'swehist2' as const, emoji: '📜', name: 'Historien', desc: 'Sveriges historia forntid till nutid — 9 sek, 10 ronder!', reward: '🪙0-170', hot: true },
  { id: 'mathest' as const, emoji: '👁️', name: 'Uppskattning', desc: 'Avrunda och uppskatta produkter — 6 sek, 12 ronder!', reward: '🪙0-180', hot: false },
  { id: 'fractarith' as const, emoji: '➗', name: 'Bråkräkning', desc: 'Addera, subtrahera och multiplicera bråk — 10 sek!', reward: '🪙0-216', hot: true },
  { id: 'worldgeo' as const, emoji: '🌍', name: 'Världen', desc: 'Huvudstäder, världsrekord och kontinenter — 8 sek!', reward: '🪙0-192', hot: false },
]
...
export const GamesView = memo(function GamesView() {
  ...
  if (activeGame === 'numbase') return <NumberBaseGame onExit={() => setActiveGame(null)} onWin={handleGenericWin} />
  if (activeGame === 'coordgame') return <CoordinateGame onExit={() => setActiveGame(null)} onWin={handleGenericWin} />
  if (activeGame === 'negnum') return <NegativeNumberGame onExit={() => setActiveGame(null)} onWin={handleGenericWin} />
  if (activeGame === 'unitconv') return <UnitConvertGame onExit={() => setActiveGame(null)} onWin={handleGenericWin} />
  if (activeGame === 'ratio') return <RatioGame onExit={() => setActiveGame(null)} onWin={handleGenericWin} />
  if (activeGame === 'speedtime') return <SpeedDistTimeGame onExit={() => setActiveGame(null)} onWin={handleGenericWin} />
  if (activeGame === 'exponents') return <ExponentsGame onExit={() => setActiveGame(null)} onWin={handleGenericWin} />
  if (activeGame === 'swwords') return <SwedishWordsGame onExit={() => setActiveGame(null)} onWin={handleGenericWin} />
  if (activeGame === 'algword') return <AlgebraWordGame onExit={() => setActiveGame(null)} onWin={handleGenericWin} />
  if (activeGame === 'swgram') return <SwedishGrammarGame onExit={() => setActiveGame(null)} onWin={handleGenericWin} />
  if (activeGame === 'multigrid') return <MultiplicationGridGame onExit={() => setActiveGame(null)} onWin={handleGenericWin} />
  if (activeGame === 'scifact') return <ScienceFactGame onExit={() => setActiveGame(null)} onWin={handleGenericWin} />
  if (activeGame === 'swehist2') return <SwedishHistoryGame onExit={() => setActiveGame(null)} onWin={handleGenericWin} />
  if (activeGame === 'mathest') return <MathEstimateGame onExit={() => setActiveGame(null)} onWin={handleGenericWin} />
  if (activeGame === 'fractarith') return <FractionArithGame onExit={() => setActiveGame(null)} onWin={handleGenericWin} />
  if (activeGame === 'worldgeo') return <WorldGeographyGame onExit={() => setActiveGame(null)} onWin={handleGenericWin} />
