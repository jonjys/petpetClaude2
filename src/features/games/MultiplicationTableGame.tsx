...
function makeQ(difficulty: number) {
  const maxTable = difficulty < 3 ? 5 : difficulty < 6 ? 9 : 12
  const table = 2 + Math.floor(Math.random() * (maxTable - 1))
  const mult = 1 + Math.floor(Math.random() * 10)
  const answer = table * mult
  const wrong1 = answer + table
  const wrong2 = Math.max(table, answer - table)
  const wrong3 = (table + 1) * mult
  const opts = [answer, wrong1, wrong2, wrong3]
    .filter((v, i, arr) => arr.indexOf(v) === i && v > 0)
    .slice(0, 4)
    .sort(() => Math.random() - 0.5)
  return { table, mult, answer, options: opts }
}

export const MultiplicationTableGame = memo(function MultiplicationTableGame({ onExit, onWin }: Props) {
  ...
  const [bestScore] = useState(() => Number(localStorage.getItem('k0509_mtg_best') ?? 0))
  ...
  const nextRound = useCallback((r: number) => {
    if (r >= ROUNDS) {
      ...
      const prev = Number(localStorage.getItem('k0509_mtg_best') ?? 0)
      if (s > prev) localStorage.setItem('k0509_mtg_best', String(s))
      onWin(s * 18, s * 55)
      setPhase('done')
      audio.achievement()
      return
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      {phase === 'ready' && (
        <div style={{ padding: '24px 16px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--t3)', maxWidth: 260, lineHeight: 1.6 }}>
            Lös multiplikationen på {TIME_LIMIT} sekunder! Tabellerna ökar i svårighet. {ROUNDS} frågor.
          </div>
      ...
      {phase === 'feedback' && (
        <div style={{ padding: '32px 16px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 900, color: '#fff' }}>{q.table} × {q.mult} = {q.answer}</div>
