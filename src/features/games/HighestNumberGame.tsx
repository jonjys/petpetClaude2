...
export const HighestNumberGame = memo(function HighestNumberGame({ onExit, onWin }: Props) {
  ...
  const [bestScore] = useState(() => Number(localStorage.getItem('k0509_hng_best') ?? 0))
  ...
  const nextRound = useCallback((r: number) => {
    if (r >= ROUNDS) {
      ...
      const prev = Number(localStorage.getItem('k0509_hng_best') ?? 0)
      if (s > prev) localStorage.setItem('k0509_hng_best', String(s))
      onWin(s * 12, s * 36)
      setPhase('done')
      audio.achievement()
      return
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div className={styles.gameHeader}>
        ...
        <span className={styles.gameTitle}>📊 Störst Vinner</span>
      ...
      {phase === 'ready' && (
        <div style={{ padding: '24px 16px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ fontFamily: 'var(--ff-head)', fontSize: 18, fontWeight: 900, color: '#fff' }}>Störst Vinner</div>
          <div style={{ fontSize: 12, color: 'var(--t3)', maxWidth: 260, lineHeight: 1.6 }}>
            Tryck på det STÖRSTA talet! {TIME_LIMIT} sekunder per fråga. {ROUNDS} ronder.
          </div>
      ...
      {phase === 'play' && (
        <div style={{ padding: '0 14px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ fontSize: 11, color: '#fbbf24', textAlign: 'center', fontWeight: 700 }}>Tryck på det STÖRSTA!</div>
      ...
      {phase === 'feedback' && (
        <div style={{ padding: '32px 16px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 900, color: '#fff' }}>Största: {q.answer}</div>
