...
function makeQ(difficulty: number) {
  const maxA = difficulty < 5 ? 20 : difficulty < 10 ? 50 : 100
  const a = 5 + Math.floor(Math.random() * maxA)
...
export const SpeedSubtractGame = memo(function SpeedSubtractGame({ onExit, onWin }: Props) {
  ...
  const [bestScore] = useState(() => Number(localStorage.getItem('k0509_ssb_best') ?? 0))
  ...
  const nextRound = useCallback((r: number) => {
    if (r >= ROUNDS) {
      ...
      const prev = Number(localStorage.getItem('k0509_ssb_best') ?? 0)
      if (s > prev) localStorage.setItem('k0509_ssb_best', String(s))
      onWin(s * 11, s * 33)
      setPhase('done')
      audio.achievement()
      return
      ...
      setTimeLeft(t => {
        if (t <= 1) {
          ...
          if (!answeredRef.current) {
            ...
            setPhase('feedback')
            audio.click()
            toRef.current = setTimeout(() => nextRound(round + 1), 600)
          }
          return 0
        }
        return t - 1
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div className={styles.gameHeader}>
        ...
        <span className={styles.gameTitle}>➖ Snabbsubtrahering</span>
      ...
      {phase === 'ready' && (
        <div style={{ padding: '24px 16px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ fontFamily: 'var(--ff-head)', fontSize: 18, fontWeight: 900, color: '#fff' }}>Snabbsubtrahering</div>
          <div style={{ fontSize: 12, color: 'var(--t3)', maxWidth: 260, lineHeight: 1.6 }}>
            Subtrahera snabbt! {TIME_LIMIT} sek per tal, {ROUNDS} ronder. Talen ökar upp till 100!
          </div>
      ...
      {phase === 'play' && (
        <div style={{ padding: '0 14px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {q.options.map((opt, i) => (
              <button key={i} onClick={() => answer(opt)} style={{ height: 64, borderRadius: 16, fontSize: 22, fontWeight: 900, background: 'rgba(248,113,113,.1)', color: accent, border: '2px solid rgba(248,113,113,.3)', cursor: 'pointer' }}>{opt}</button>
            ))}
          </div>
        </div>
      )}
      ...
      {phase === 'done' && (
        <div style={{ padding: '24px 16px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ fontSize: 13, color: '#fbbf24' }}>+{score * 11}🪙 +{score * 33} XP</div>
