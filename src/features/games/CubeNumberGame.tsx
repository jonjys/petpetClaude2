...
function makeQ(difficulty: number) {
  const max = difficulty < 5 ? 5 : difficulty < 9 ? 8 : 10
  const n = 2 + Math.floor(Math.random() * (max - 1))
  const answer = n * n * n
  const makeWrong = () => {
    const delta = 1 + Math.floor(Math.random() * 3)
    const wrong = Math.max(2, n + (Math.random() < 0.5 ? delta : -delta))
...
export const CubeNumberGame = memo(function CubeNumberGame({ onExit, onWin }: Props) {
  ...
  const [bestScore] = useState(() => Number(localStorage.getItem('k0509_cub_best') ?? 0))
  ...
  const nextRound = useCallback((r: number) => {
    if (r >= ROUNDS) {
      ...
      const prev = Number(localStorage.getItem('k0509_cub_best') ?? 0)
      if (s > prev) localStorage.setItem('k0509_cub_best', String(s))
      onWin(s * 15, s * 45)
      setPhase('done')
      audio.achievement()
      return
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div className={styles.gameHeader}>
        ...
        <span className={styles.gameTitle}>³ Kubning</span>
      ...
      {phase === 'ready' && (
        <div style={{ padding: '24px 16px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ fontFamily: 'var(--ff-head)', fontSize: 18, fontWeight: 900, color: '#fff' }}>Kubning</div>
          <div style={{ fontSize: 12, color: 'var(--t3)', maxWidth: 260, lineHeight: 1.6 }}>
            Vad är n³? Välj rätt svar på {TIME_LIMIT} sekunder! {ROUNDS} ronder — tal ökar till 10!
          </div>
      ...
      {phase === 'play' && (
        <div style={{ padding: '0 14px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {q.options.map((opt, i) => (
              <button key={i} onClick={() => answer(opt)} style={{ height: 64, borderRadius: 16, fontSize: 18, fontWeight: 900, background: 'rgba(167,139,250,.1)', color: accent, border: '2px solid rgba(167,139,250,.3)', cursor: 'pointer' }}>{opt}</button>
            ))}
          </div>
        </div>
      )}
